...
const userCommandCooldowns = new Map(); // key: userId-command, value: timestamp
async function processCommand(ctx, bot, telegraf) {
    ...
    const { data: commands, error } = await supabase_1.supabase
    ...
    if (command) {
        ...
        if (commandName === 'start' && bot.module_welcome) {
            ...
            const { data: welcomeTemplate } = await supabase_1.supabase
            ...
            // Load help template
            const { data: helpTemplate } = await supabase_1.supabase
                .from('telegram_bot_templates')
                .select('*')
                .eq('bot_id', bot.id)
