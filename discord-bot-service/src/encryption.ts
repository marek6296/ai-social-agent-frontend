 ...
 * @returns Decrypted token or empty string if decryption fails
 */
export function decryptToken(encryptedText: string): string {
  ...
  // Discord tokens have format: XXXX.XXXX.XXXX and contain dots
  // They're usually 50-100 characters long
  ...
  if (!isLikelyEncrypted) {
    ...
    // Discord tokens can have various formats, so we'll try using it as-is
    return encryptedText;
  }
  ...
  try {
    ...
    if (decryptedText.includes('.') && 
      ...
      // This likely means wrong encryption key was used, but decryption didn't fail
      // (decryption with wrong key can produce garbage that doesn't fail)
      console.warn('⚠️ Decryption succeeded but result does not look like valid Discord token');
      console.warn('   This likely means token was encrypted with different key');
      console.warn('   Returning original encrypted text - bot manager will reject it');
      return encryptedText; // Return original encrypted text
    }
  } catch (error) {
    // Decryption failed - token might be:
    // 1. Plain text (not encrypted) but unusual format
    // 2. Encrypted with different key
    // 3. Invalid format
    // Return original text - we'll let the bot manager try to use it
    // (Discord.js will fail with proper error if token is invalid)
