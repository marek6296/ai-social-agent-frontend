...
export default function ModerationPage() {
  ...
  const getTriggerLabel = (triggerType: string, config: any) => {
    switch (triggerType) {
      case "keyword_match":
        return `Keywords: ${Array.isArray(config?.keywords) ? config.keywords.join(", ") : ""}`;
      case "spam_detection":
  ...
  const getActionLabel = (actions: any[]) => {
    ...
    if (firstAction.type === "warn") {
      return "Varovanie";
    }
    if (firstAction.type === "timeout") {
      return `Timeout: ${firstAction.config?.duration || ""}`;
    }
    if (firstAction.type === "delete") {
      return "Vymazať správu";
    }
    if (firstAction.type === "kick") {
      return "Vyhodiť";
    }
    if (firstAction.type === "ban") {
      return "Zabanovať";
    }
    ...
    <>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold">Moderácia</h1>
            <p className="text-muted-foreground mt-1">
              Automod pravidlá a moderácia automatizácia
            </p>
          </div>
          <Button onClick={handleNewFlow}>
            <Plus className="h-4 w-4 mr-2" />
            Pridať moderáciu flow
          </Button>
        </div>
      </div>
        ...
        <Card>
          <CardContent className="pt-12 pb-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">
              Ešte nemáš žiadne moderácia flows
            </h3>
            <p className="text-muted-foreground mb-4">
              Vytvor moderácia flow pre automod, anti-spam, anti-invite a ďalšie ochranné pravidlá
            </p>
            <Button onClick={handleNewFlow}>
              <Plus className="h-4 w-4 mr-2" />
              Pridať prvý moderácia flow
            </Button>
          </CardContent>
        </Card>
      ) : (
