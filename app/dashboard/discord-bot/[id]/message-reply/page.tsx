...
export default function MessageReplyPage() {
    ...
    <>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold">Odpovedanie na správy</h1>
            <p className="text-muted-foreground mt-1">
              Nastav, kedy a ako má bot odpovedať na správy
            </p>
          </div>
      ...
      {flows.length === 0 ? (
        <Card>
          <CardContent className="pt-12 pb-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">
              Ešte nemáš žiadne flows
            </h3>
            <p className="text-muted-foreground mb-4">
              Vytvor svoj prvý flow, aby bot vedel odpovedať na správy
            </p>
            <Button onClick={handleNewFlow}>
              <Plus className="h-4 w-4 mr-2" />
              Pridať prvý flow
        ...
        <div className="grid gap-4">
          {flows.map((flow) => (
            <Card
              key={flow.id}
              className={!flow.enabled ? "opacity-60" : ""}
            >
              <CardHeader>
